...
def youtube_search(query, max_results=5):
    ...
    # Check if there are no results, then broaden the search
    if not response['items']:
        print("No videos found, trying a broader search.")
        request = youtube.search().list(
            part="snippet",
            q="general " + query,  # Broaden search by adding "general"
            maxResults=max_results + 5  # Increase results as fallback
        )
        response = request.execute()
...
def video_story():
    ...
    if s3_url:
        # Send the S3 URL to the external endpoint
        ...
        # Get the returned video URL from the response
        if response.status_code == 200:
